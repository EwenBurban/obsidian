...
rf_posterior = file.path(grep(ld,pattern=pattern,value=T),'posterior_SI_1N.txt')
rf_data = do.call(rbind,lapply(rf_posterior,read.table,h=T))[,-1]
...
ggarrange(plotlist=plotlist,nrow=2,ncol=2) %>% ggexport(filename=file.path(output_dir,'SI_1N_posterior_estimation_v2.jpg'),width=1400,height=1400,units='px')
...
rf_posterior = file.path(grep(ld,pattern=pattern,value=T),'posterior_SI_2N.txt')
rf_data = do.call(rbind,lapply(rf_posterior,read.table,h=T))[,-1]
rf_data$tag = 'post_abcrf'
lf_data = rbind(lf_data,rf_data)
prior_ld = list.dirs(prior_dir,recursive=F)
...
ggarrange(plotlist=plotlist,nrow=2,ncol=2) %>% ggexport(filename=file.path(output_dir,'SI_2N_posterior_estimation_v2.jpg'),width=1400,height=1400,units='px')
